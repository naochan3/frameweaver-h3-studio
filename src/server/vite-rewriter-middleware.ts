...
export function createViteRewriterMiddleware(gateway: (request: Request) => Promise<Response>) {
  return async (request: IncomingMessage, response: ServerResponse, next: Next) => {
    ...
    try {
      const body = request.method === 'GET' || request.method === 'HEAD' ? undefined : await readBody(request, 16 * 1024)
